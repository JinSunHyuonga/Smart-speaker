...
static int vumeter = VUMETER_MONO;
...
#ifdef PRINTF
		printf("(set_params)bits per sample : %u\n", bits_per_sample);
#endif

		bits_per_frame = bits_per_sample * hwparams.channels;

#ifdef PRINTF
		printf("(set_params)bits per frame : %u\n", bits_per_frame);
#endif
...
static void compute_max_peak(u_char *data, size_t count)
{
...
#ifdef xPRINTF
						printf("(compute_max_peak)mask8 : %d\n", mask);
#endif
		...
		case 16: {
						...
						signed short sval;
#ifdef xPRINTF
						printf("(compute_max_peak)mask16 : %d\n", mask);
#endif

						count /= 2;

#ifdef PRINTF
						printf("(compute_max_peak)count : %d\n", count);
#endif
						c = 0;
						while (count-- > 0) {
		...
		case 32: {
						...
						count /= 4;
						c = 0;
/////////////////////*read amplitude from data array*/
						while (count-- > 0) {
								if (format_little_endian)
...
#ifdef PRINTF
		printf("(compute_max_peak)bits_per_sample : %u\n", bits_per_sample);
#endif

		max = 1 << (bits_per_sample-1);

#ifdef PRINTF
		printf("(compute_max_peak)max : %d\n", max);
#endif
...
#ifdef PRINTF
		printf("(compute_max_peak)perc[%i] : %i\n", c, perc[c]);
#endif
		...
		if (interleaved && verbose <= 2) {
				...
				putchar('\r');
#ifdef PRINTF
		printf("\n(compute_max_peak)maxperc[0] : %i\n", maxperc[0]);
		printf("(compute_max_peak)maxperc[1] : %i\n\n", maxperc[1]);
#endif
		vu_level = maxperc[0];
...
static void capture(char *orig_name)
{
		...
		do {
...
#ifdef PRINTF
				printf("(capture)rest chunk bpf: %u %u %u\n", rest, chunk_bytes, bits_per_frame);
#endif

				/* capture */
				fdcount = 0;
				time_t timer0, timer1, inter0, inter1, dt_val;
				inter0 = time(&timer0);
			...
			capture_stop == 0);
			printf("\narecord: Stopping capturing audio.\n");
}
